...
function extractFile(root, langKey) {
  ...
  const symbols = [];
  const calls = [];
  // Every identifier-shaped token in the file, and the exact nodes that ARE
  // definition names. The receipt verifier's graph refinement is valid ONLY
  // when the graph's occurrence list for a defined name is complete — and
  // "recorded only in call-argument position" left `module.exports = { pay }`,
  // `export { pay }`, `return pay` and `const x = pay` invisible, so receipts
  // citing genuine code were rejected as mentions. Rather than chasing node
  // types one syntax at a time (the CHI-R137 class), completeness is had by
  // construction: after the walk, every identifier token matching a name
  // DEFINED in this file becomes an occurrence.
  const identifiers = [];
  const defNameNodes = new Set();
  ...
  (function walk(node) {
    ...
    if (node.type.endsWith('identifier') && node.text) {
      identifiers.push({ text: node.text, line: node.startPosition.row + 1, id: node.id });
    }
  ...
  // Occurrence completeness (see the walk comment): every identifier token
  // whose text matches a symbol DEFINED in this file is an occurrence — a
  // reference is a use, so it rides in `calls` exactly like the argument
  // references above. The definition's own name token is excluded (it is the
  // symbol record itself), and (name, line) pairs already recorded are not
  // duplicated. Cross-file references keep coming from the argument block:
  // this sweep only needs to be complete for names the refinement can fire
  // on, and the refinement fires only on names defined in the same file.
  const definedNames = new Set(symbols.map(s => s.name));
  const callKeys = new Set(calls.map(c => `${c.name} ${c.line}`));
  for (const t of identifiers) {
    if (defNameNodes.has(t.id)) continue;
    const name = t.text.slice(0, 200);
    if (!definedNames.has(name)) continue;
    const key = `${name} ${t.line}`;
    if (callKeys.has(key)) continue;
    callKeys.add(key);
    calls.push({ name, line: t.line });
  }
  ...
  // covers it, and each symbol to its innermost enclosing container. A
  // line-sorted sweep with an active-interval list — the full rescan per
  // query was O((symbols+calls) * symbols), ~1e8 comparisons at the caps.
  // Semantics identical to the rescan: among covering intervals, smallest
  // span wins, ties broken by symbols-array order (the active list is kept
  // in that order).
  const queries = [];
  symbols.forEach((s, i) => queries.push({ line: s.line, symIdx: i, call: null }));
  for (const call of calls) queries.push({ line: call.line, symIdx: -1, call });
  queries.sort((a, b) => a.line - b.line);
  let ptr = 0;
  let active = [];
  for (const q of queries) {
    while (ptr < symbols.length && symbols[ptr].line <= q.line) { active.push(ptr); ptr++; }
    active = active.filter(i => symbols[i].end_line >= q.line);
    let best = -1, bestSpan = Infinity;
    for (const i of active) {
      if (i === q.symIdx) continue;
      const span = symbols[i].end_line - symbols[i].line;
      if (span < bestSpan) { best = i; bestSpan = span; }
    }
    if (q.call) {
      q.call.from = best === -1 ? null : symbols[best].name;
    } else if (best !== -1) {
      const s = symbols[q.symIdx];
      // A container must strictly contain, not merely start on the same line.
      if (!(symbols[best].line === s.line && symbols[best].end_line === s.end_line)) {
        s.container = symbols[best].name;
      }
    }
  }
...
// Transitive impact: every symbol whose behavior can change when `name`
// changes — reverse BFS over the call graph (direct callers, their callers,
// and so on up to every root). callersOf answers "who touches this";
// impactOf answers "what breaks". Excludes the symbol itself.
function impactOf(index, name) {
  const rev = new Map(); // callee -> Set(caller)
  for (const f of index.files) {
    for (const c of f.calls) {
      const from = c.from === null || c.from === undefined ? '(top)' : c.from;
      if (!rev.has(c.name)) rev.set(c.name, new Set());
      rev.get(c.name).add(from);
    }
  }
  const seen = new Set();
  const queue = [name];
  while (queue.length > 0) {
    const n = queue.shift();
    for (const caller of rev.get(n) || []) {
      if (!seen.has(caller)) { seen.add(caller); queue.push(caller); }
    }
  }
  seen.delete(name);
  return seen;
...
async function cmdGraph(cwd, log = console.log) {
  try {
    ...
    ogmaWrite(cwd, 'ogham/graph/index.json', JSON.stringify(index, null, 2) + '\n');
    ...
    // A repo with no commits fails rev-parse with git internals in the message
    // (listTracked throws before the tracked-files check runs); terrain and
    // watch translate the same condition, and graph must not be the one entry
    // point that leaks the raw git error.
    if (/rev-parse failed/.test(e.message)) {
      log('ogma graph failed: not a git repository with commits — OGMA reads only committed state, so commit something first.');
      return 1;
    }
...
module.exports = {
  ...
  definitionsOf, callersOf, trace, reachableFrom, impactOf,
