...
function cmdTerrain(cwd, log = console.log) {
  try {
    ...
    const { commit, entries, submodules } = listTracked(cwd);
    if (entries.length === 0) {
    ...
    const readText = (p) => {
      try { return git(cwd, ['show', `HEAD:${p}`], MAX_MANIFEST_BYTES + 1024); }
      catch { return null; }
    };
    ...
    fs.writeFileSync(terrainPath, JSON.stringify(result, null, 2) + '\n');
...
module.exports = { cmdTerrain, analyzeTree, mergeTerrain, MIN_MODULE_FILES, EXCLUDED_DIRS, LANG_BY_EXT };
