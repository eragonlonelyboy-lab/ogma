...
function mergeTerrain(existing, scanned) {
  ...
  // When a scanned surface's id collides with an existing one it is renamed
  // (web -> web-2). Its scanned MODULES still carry the OLD id in their
  // surface_ids, so without this map they would be re-attributed to the
  // pre-existing surface that happened to own the name — guides would then
  // document the wrong app's features under a passing certificate. The map
  // is keyed by root, because two scanned surfaces can share the same
  // colliding id and only the root tells them apart.
  const scannedSurfaceIdByRoot = new Map();
  for (const s of scanned.surfaces) {
    if (knownSurfaceRoots.has(s.root)) {
      // Root already known: the scanned module maps onto whatever existing
      // surface owns that root, under that surface's real id.
      const owner = surfaces.find(x => x.root === s.root);
      if (owner) scannedSurfaceIdByRoot.set(s.root, owner.id);
      continue;
    }
    const id = uniqueId(s.id, knownSurfaceIds);
    surfaces.push({ ...s, id });
    scannedSurfaceIdByRoot.set(s.root, id);
    added.push(`surface ${id} (${s.root})`);
  }
  // Rebuild the scanned-id -> final-id map by pairing each scanned surface
  // (its original id) with the final id its root resolved to.
  const remapSurfaceId = new Map();
  for (const s of scanned.surfaces) {
    const finalId = scannedSurfaceIdByRoot.get(s.root);
    if (finalId) remapSurfaceId.set(s.id, finalId);
  }
  ...
  for (const m of scanned.modules) {
    ...
    // Remap the module's surface_ids through the rename map BEFORE the
    // membership check, so a module of a renamed surface is added under the
    // surface it actually belongs to, not the one that stole its name.
    const mapped = (m.surface_ids || []).map(id => remapSurfaceId.get(id) || id);
    if (!mapped.every(id => surfaceIds.has(id))) continue;
    const id = uniqueId(m.id, knownModuleIds);
    modules.push({ ...m, id, surface_ids: mapped });
