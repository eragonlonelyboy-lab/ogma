// The Gate — nine deterministic checks and the certificate. This is the
...
const { verifyReceipt, makeGitReader, containsWord } = require('./verify');
const { factInputHash } = require('./witness');
const { loadOgham, parseAnnotations, stripAnnotations } = require('./render');
...
function narrativeText(markdown) {
...
function leakHits(markdown, extraTerms) {
  const text = narrativeText(markdown).toLowerCase();
  const hits = [];
...
function cmdGate(cwd, log = console.log) {
  try {
    ...
    if (head.error || head.status !== 0) { log('ogma gate failed: not a git repository with commits.'); return 1; }
    ...
    // surfaces get no guide) are built into outDocuments and recorded here.
    {
      ...
      if (o.config.audiences.prd === true) {
        const prd = readOut('prd.md') || '';
        const absent = o.terrain.modules.filter(m => !prd.includes(`## ${m.name}`)).map(m => m.id);
    ...
    // against the repo at the fact's own commit.
    {
      const readers = new Map();
      const readerAt = (c) => {
        if (!readers.has(c)) {
          const raw = makeGitReader(cwd, c);
          const cache = new Map();
          readers.set(c, (p) => { if (!cache.has(p)) cache.set(p, raw(p)); return cache.get(p); });
        }
      ...
      for (const f of allFacts) {
        ...
        const receipts = [
          ...(Array.isArray(f.receipts) ? f.receipts : []),
          ...(f.path && Array.isArray(f.path.chain) ? f.path.chain.map(h => h && h.receipt).filter(Boolean) : [])
        ];
        for (const r of receipts) {
          const v = verifyReceipt(r, reader);
      ...
      for (const q of Array.isArray(o.ledger.questions) ? o.ledger.questions : []) {
        for (const r of Array.isArray(q.receipts) ? q.receipts : []) {
          const v = verifyReceipt(r, readerAt(headCommit));
    ...
    {
      const hits = [];
      const docs = ['prd.md', ...o.terrain.surfaces
        .filter(s => S.INTERACTIVE_SURFACE_KINDS.includes(s.kind))
        .map(s => `guides/${s.id}.md`)];
      ...
      record('leaklint', hits.length === 0, hits.length ? hits.slice(0, 10).join('; ') : 'no technical vocabulary reached non-technical readers');
    }
    ...
    // annotation nothing owns is a fabricated claim wearing a receipt).
    {
      ...
      const walk = (dir, rel = '') => {
        ...
        for (const e of fs.readdirSync(dir)) {
          ...
          if (fs.statSync(full).isDirectory()) outDocs.push(...walk(full, rel ? `${rel}/${e}` : e));
          else if (e.endsWith('.md')) outDocs.push(rel ? `${rel}/${e}` : e);
        }
        return outDocs;
      };
    ...
    fs.writeFileSync(path.join(cwd, '.ogma', 'certificate.json'), JSON.stringify(certificate, null, 2) + '\n');

    const badge = pass ? 'PASS' : 'FAIL';
    log(`Certificate: ${badge} at ${headCommit.slice(0, 12)} — ${checks.filter(c => c.pass).length}/9 checks green.`);
