...
function cmdInit(cwd, log = console.log) {
  ...
  try {
    ...
    // A self-ignoring workspace. OGMA's state describes the repo; it is not
    // part of it, and it must never arrive WITH a repo — a fact file that came
    // from the cloned repository would otherwise be read as one this operator's
    // agent authored, certified, and published. Ignoring `*` from inside .ogma
    // means an ordinary `git add -A` cannot track any of it, so tracked OGMA
    // state is thereafter a deliberate act and the readers refuse it (see
    // trackedOgmaState). Written first, before any state exists to be caught.
    const ignorePath = path.join(ogmaDir, '.gitignore');
    const ignSt = refuseSymlink(ignorePath);
    if (ignSt) {
      if (!ignSt.isFile()) throw new Error('cannot create .ogma/.gitignore: a directory with that name already exists');
      kept.push('.gitignore');
    } else {
      fs.writeFileSync(ignorePath, '# OGMA state is local to this checkout and never committed.\n*\n', { flag: 'wx' });
