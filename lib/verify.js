...
function verifyReceipt(receipt, readFile, index) {
  ...
  if (index && Array.isArray(index.files)) {
    const gf = index.files.find(f => f.path === receipt.file);
    if (gf) {
      const occurrences = [];
      for (const s of gf.symbols) if (s.name === receipt.symbol) occurrences.push(s.line);
      for (const c of gf.calls) if (c.name === receipt.symbol) occurrences.push(c.line);
