...
// The Unicode normalization rule (pinned in docs/ogham-schema.md — this file
// and that section change together): macOS writes filenames in NFD while
// almost everything else authors NFC, so the same visible name can be two
// byte sequences. Comparisons between Ogham strings and repo-derived strings
// are NFC-normalized on BOTH sides, and a git path lookup whose stored bytes
// miss retries the NFD and NFC forms before ruling the file missing. Without
// this, every receipt into an accented path is a false broken receipt on the
...
// An optional graph index (graph/index.json shape) upgrades the check from
// text-backed to symbol-table-backed — see the refinement block below.
function verifyReceipt(receipt, readFile, index) {
  const structural = [];
  ...
  if (!containsWord(nfc(window), nfc(receipt.symbol))) {
    return {
      ok: false, reason: 'symbol-not-found',
      detail: `"${safe(receipt.symbol, 80)}" not found as a whole word in ${safe(receipt.file, 200)}:${lo}-${hi}`
    };
  }
  // Symbol-table refinement, when a graph index is supplied. Text matching
  // cannot tell a definition from a mention in a comment; the graph can. The
  // rule TIGHTENS and never loosens: if the graph knows this symbol in this
  // file (as a definition or a call site), at least one of those real
  // occurrences must sit inside the drift window — a comment mention within
  // the window no longer verifies. If the graph does not know the symbol
  // (variables, config keys, unsupported languages), the text verdict
  // stands: the graph's ignorance is never an excuse to reject a receipt.
  if (index && Array.isArray(index.files)) {
    const wantPath = nfc(receipt.file);
    const wantSymbol = nfc(receipt.symbol);
    const gf = index.files.find(f => nfc(f.path) === wantPath);
    if (gf) {
      // The tightening is valid ONLY when the graph's occurrence list for
      // this name in this file is complete, and that requires a DEFINITION
      // entry: the indexer records function/class/method definitions plus
      // call and reference sites, but never variable declarations. A name
      // the graph knows only through call/reference sites therefore has a
      // definition the graph cannot see (a const referenced as an argument,
      // an imported function) — rejecting the definition's own receipt
      // against that incomplete list is the graph's ignorance rejecting a
      // receipt, which the rule forbids. First found by the OSS-fixture
      // benchmark: `const mo = y / 12` was uncitable because `mo` appeared
      // later as a bare argument.
      const defined = gf.symbols.some(s => nfc(s.name) === wantSymbol);
      const occurrences = [];
      for (const s of gf.symbols) if (nfc(s.name) === wantSymbol) occurrences.push(s.line);
      for (const c of gf.calls) if (nfc(c.name) === wantSymbol) occurrences.push(c.line);
      if (defined && occurrences.length > 0 && !occurrences.some(l => l >= lo && l <= hi)) {
        return {
          ok: false, reason: 'symbol-not-found',
          detail: `"${safe(receipt.symbol, 80)}" appears in ${safe(receipt.file, 200)} only at line(s) ` +
            `${occurrences.slice(0, 5).join(', ')} per the graph — outside the cited window ${lo}-${hi}; ` +
            'the text match inside the window is a mention, not the code'
        };
      }
      return { ok: true, via: defined ? 'graph' : 'text' };
    }
  }
...
function verifyReceipts(receipts, readFile, index) {
  const cache = new Map();
  const cachedRead = (p) => {
  ...
  for (const r of list) {
    const v = verifyReceipt(r, cachedRead, index);
    if (!v.ok) failed++;
    results.push(v);
  }
...
function makeGitReader(cwd, commit) {
  ...
  const readAt = (fileArg) => {
    // Type before size. `cat-file -s` succeeds on a TREE, and `git show` then
    // returns its directory listing, which would be handed back as file
    // content: a receipt citing `lib` would "verify" because the listing
    // happens to contain the word `gate.js`, and a blind judge would be shown
    // a directory listing as the cited code. Only a blob is a file.
    const type = spawnSync('git', ['-C', cwd, 'cat-file', '-t', `${commit}:${fileArg}`], { encoding: 'utf8' });
    if (type.error || type.status !== 0) return null;
    if (type.stdout.trim() !== 'blob') return null;
    const probe = spawnSync('git', ['-C', cwd, 'cat-file', '-s', `${commit}:${fileArg}`], { encoding: 'utf8' });
    if (probe.error || probe.status !== 0) return null;
    const size = Number(probe.stdout.trim());
    if (Number.isFinite(size) && size > MAX_SOURCE_BYTES) return { toolarge: true };
    const r = spawnSync('git', ['-C', cwd, 'show', `${commit}:${fileArg}`],
      { encoding: 'utf8', maxBuffer: MAX_SOURCE_BYTES + 1024 });
    if (r.error || r.status !== 0) return null;
    return { content: r.stdout };
  };
  return (file) => {
    // Stored bytes first; NFD and NFC forms only if they differ. A miss on
    // all forms is a genuinely missing file, not a normalization artifact.
    const variants = [String(file)];
    for (const v of [String(file).normalize('NFD'), String(file).normalize('NFC')]) {
      if (!variants.includes(v)) variants.push(v);
    }
    for (const v of variants) {
      const r = readAt(v);
      if (r !== null) return r.toolarge ? r : r.content;
    }
    return null;
  };
}

// Per-commit cached readers over one repo. Facts cluster at a handful of
// commits and their receipts cluster in the same files; without both cache
// layers the gate goes quadratic. Was two identical private copies in ingest
// and gate.
function makeReaderCache(cwd) {
  const readers = new Map(); // commit -> cached reader
  return (commit) => {
    if (!readers.has(commit)) {
      const raw = makeGitReader(cwd, commit);
      const cache = new Map();
      readers.set(commit, (p) => {
        if (!cache.has(p)) cache.set(p, raw(p));
        return cache.get(p);
      });
    }
    return readers.get(commit);
  };
}

module.exports = { verifyReceipt, verifyReceipts, makeGitReader, makeReaderCache, containsWord, toLines, MAX_SOURCE_BYTES };
