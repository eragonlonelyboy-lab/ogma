...
function worstClassification(classifications) {
  ...
  for (const c of classifications) {
    if (!(c in SEVERITY)) throw new Error(`worstClassification: unknown classification "${c}"`);
...
function validateWitness(w, errors, ctx) {
  ...
  if (!isNonEmptyString(w.checked_at_commit, 64)) {
    errors.push(`${ctx}: witness.checked_at_commit missing`);
  }
}
...
function validateFeature(feat, errors) {
  ...
  if (feat.classification === 'LIVE') {
    for (const k of ['does', 'happens', 'sees']) {
      if (!isNonEmptyString(feat[k])) errors.push(`${ctx}: ${k} is empty — every LIVE feature carries does/happens/sees`);
    }
  } else if (!isNonEmptyString(feat.why_not_reachable)) {
    errors.push(`${ctx}: non-LIVE features carry why_not_reachable instead of user-facing narration`);
  }
...
function validateModuleFile(doc, errors) {
  if (!isObject(doc)) { errors.push('module file: not an object'); return; }
  ...
  for (const feat of features) {
    ...
    // Reverse direction: every fact pointing here must be listed.
    for (const fact of facts) {
      if (fact.feature_id === feat.id && !feat.fact_ids.includes(fact.id)) {
