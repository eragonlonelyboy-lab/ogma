...
const WITNESS_HASH_VERSION = 'ogma-witness-v1';
...
function rendersTo(fact, audience) {
  ...
  return fact.classification === 'LIVE';                              // business and guides: LIVE only
}
...
function witnessInputHash(statement, excerpts) {
  ...
  const canonical = [WITNESS_HASH_VERSION, stmt, ...blocks].join('\n--\n');
...
function validatePathArray(arr, errors, ctx, fieldName, { required = true } = {}) {
  if (!Array.isArray(arr)) { errors.push(`${ctx}: ${fieldName} must be an array`); return; }
  if (required && arr.length === 0) { errors.push(`${ctx}: ${fieldName} must not be empty`); return; }
  if (arr.length > MAX_ARRAY) { errors.push(`${ctx}: ${fieldName} exceeds ${MAX_ARRAY} entries`); return; }
  arr.forEach((p, i) => {
    if (!isSafeRepoPath(p)) errors.push(`${ctx}: ${fieldName}[${i}] must be a contained repo-relative POSIX path`);
  });
}
...
function validateTerrain(t, errors) {
  ...
  if (!Array.isArray(t.surfaces) || t.surfaces.length === 0) {
    ...
    t.surfaces.forEach((s, i) => {
      ...
      if (!isSafeRepoPath(s.root)) errors.push(`${ctx}: root must be a contained repo-relative path`);
  ...
  if (!Array.isArray(t.modules)) {
    ...
    t.modules.forEach((m, i) => {
      ...
      validatePathArray(m.roots, errors, ctx, 'roots');
