...
const { readJson, ogmaWrite, safeListFiles } = require('./util');
...
function loadOgham(cwd) {
  ...
  if (fs.existsSync(factsDir)) {
    for (const f of safeListFiles(factsDir)) {
...
function buildPrd(o) {
  ...
  // Says exactly what is true and no more. The old line promised that EVERY
  // claim traces to verified code, but the feature summaries below
  // (does/happens/sees) are authored prose: they carry no receipt and no
  // witness ruling, and no gate check reaches them. Only the rules do. The
  // shorter sentences also drop this preamble from grade 11.1 to 5.5, so a
  // PRD with few features no longer fails readability on OGMA's own boilerplate.
  lines.push(`_Current as of commit ${o.manifest.cutoff_commit.slice(0, 12)}. Each rule below was checked against the code it names. The short summaries are written from those rules and are not checked on their own. Open points are listed in questions.md._`, '');
