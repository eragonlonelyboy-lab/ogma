...
function outDir(cwd) {
  const d = path.join(cwd, '.ogma', 'out');
  if (!fs.existsSync(d)) fs.mkdirSync(d, { recursive: true });
  return d;
}

function writeDoc(cwd, rel, content) {
  const full = path.join(outDir(cwd), rel);
...
function cmdPrd(cwd, log = console.log) {
  ...
  writeDoc(cwd, 'prd.md', lines.join('\n') + '\n');
...
function cmdExplain(cwd, log = console.log) {
  ...
  for (const m of o.terrain.modules) {
    ...
    writeDoc(cwd, `tech/${m.id}.md`, lines.join('\n') + '\n');
    wrote++;
  }
  log(`Wrote ${wrote} module note(s) under .ogma/out/tech/`);
  return 0;
}
...
function cmdGuides(cwd, log = console.log) {
  ...
  for (const s of o.terrain.surfaces) {
    ...
    writeDoc(cwd, `guides/${s.id}.md`, lines.join('\n') + '\n');
    wrote++;
  }
  log(`Wrote ${wrote} guide(s) under .ogma/out/guides/`);
  return 0;
}
...
function cmdQuestions(cwd, log = console.log) {
  ...
  writeDoc(cwd, 'questions.md', lines.join('\n') + '\n');
  log(`Wrote .ogma/out/questions.md (${open.length} open, ${closed.length} resolved)`);
  return 0;
}
