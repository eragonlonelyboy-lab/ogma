...
const { loadOgham } = require('./render');
...
function buildViews(o) {
  ...
  for (const m of o.terrain.modules) {
    ...
    const features = (Array.isArray(doc.features) ? doc.features : []).map(feat => {
      ...
      return {
        ...
        narratable: own.some(f => S.rendersTo(f, 'prd')),
  ...
  try {
    const cert = JSON.parse(fs.readFileSync(path.join(o.cwd, '.ogma', 'certificate.json'), 'utf8'));
    certificate = { pass: cert.pass === true, commit: cert.commit, checks: cert.checks.map(c => ({ check: c.check, pass: c.pass })) };
  } catch { /* no certificate yet — shown as UNCERTIFIED */ }
  return {
...
function renderMd(v) {
  ...
  lines.push(`Certificate: ${v.certificate ? (v.certificate.pass ? 'PASS' : 'FAIL') : 'not yet run'}`, '');
  lines.push('## Surfaces', '');
...
function render() {
  ...
  DATA.modules.forEach(function (m) {
    ...
    m.features.forEach(function (f) {
      ...
      if (audience !== 'tech' && facts.length === 0 && !f.narratable) return;
      shown++;
      var box = el('div', 'feat');
...
function cmdMap(cwd, log = console.log) {
  ...
  o.cwd = cwd;
  const v = buildViews(o);
  const out = path.join(cwd, '.ogma', 'out');
  if (!fs.existsSync(out)) fs.mkdirSync(out, { recursive: true });
  fs.writeFileSync(path.join(out, 'map.md'), renderMd(v));
  fs.writeFileSync(path.join(out, 'map.html'), renderHtml(v));
  const canvas = renderCanvas(v);
  JSON.parse(canvas);   // a canvas that does not parse is not written half-broken
  fs.writeFileSync(path.join(out, 'map.canvas'), canvas);
