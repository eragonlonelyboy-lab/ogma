...
function writeState(statePath, kind, commit, files, log) {
  const state = {
...
async function cmdPush(cwd, rest = [], log = console.log, env = process.env) {
  try {
    ...
    if (kind === 'markdown-only') {
      ...
      try {
        ...
        if (delivered.length > 0) {
          ...
          writeState(statePath, kind, headCommit, [...delivered, ...kept], log);
          log(`ogma push failed after ${delivered.length} verified deliver(ies) — their page mapping is recorded; the rest keeps its prior state.`);
        }
    ...
    // --- record --------------------------------------------------------------
    if (!writeState(statePath, kind, headCommit, delivered, log)) return 1;
