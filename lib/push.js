...
const { readJson, nowIso, ogmaWrite, escapeXml, gitHead } = require('./util');
...
function loadCertifiedFleet(cwd) {
  ...
  for (const rel of expected) {
    ...
    // gate and push must not ship under the certified banner. map.md/html/
    // canvas are views over the certificate itself (legitimately regenerated
    // after the gate) and are exempt by design — the certificate does not
    // list them.
    if (!/^map\./.test(rel)) {
...
async function cmdPush(cwd, rest = [], log = console.log, env = process.env) {
  try {
    ...
    // Provenance gate, BEFORE consent is read or recorded. `init` refuses a
    // repo-supplied destination, but push can run without init ever having
    // run — and a repo can ship .ogma/config.json (a pre-seeded consent or
    // targeting block) or .ogma/push-state.json (attacker page-id mappings a
    // replay would UPDATE). A file the repo tracks is a file the repo's
    // author wrote, not a choice this operator made; consent and delivery
    // records are trusted only when this tool wrote them locally.
    //
    // Deliberately NOT extended to the whole of .ogma/. A repo-shipped
    // certificate.json cannot authorize anything: the certificate must name
    // the commit that contains it (loadCertifiedFleet's sameCommit check
    // against HEAD), and committing a file changes the commit hash that file
    // would have to name — an unforgeable fixpoint, pinned by the batch-10
    // bench check so it can never quietly become incidental. Refusing every
    // tracked .ogma/ file would instead break the ordinary `git add -A`
    // workflow, since init deliberately writes no .gitignore.
    const tracked = spawnSync('git', ['-C', cwd, 'ls-files', '--', '.ogma/config.json', '.ogma/push-state.json'],
      { encoding: 'utf8' });
    const trackedFiles = tracked.status === 0 ? tracked.stdout.split('\n').filter(Boolean) : [];
    if (trackedFiles.length > 0) {
      log(`ogma push refused: ${trackedFiles.join(' and ')} ${trackedFiles.length > 1 ? 'are' : 'is'} tracked in this repository.`);
      log('A repo-supplied delivery config or delivery record is never trusted — consent is recorded by this tool, not adopted from a file it did not write.');
      log('Untrack with `git rm --cached <file>`, commit, then re-record your destination with `ogma push --to ...`.');
      return 1;
    }
