...
const { readJson, nowIso, ogmaWrite, gitHead, safeListFiles, MAX_REPORTED, trackedOgmaState, refuseTrackedOgma } = require('./util');
...
function cmdIngest(cwd, log = console.log) {
  try {
    ...
    // Provenance before anything is read as authored. This is the earliest
    // point a repo-supplied fact file could enter the pipeline, and ingest is
    // what stamps the manifest that binds the Ogham to HEAD — the one piece a
    // hostile repo cannot forge for itself.
    const tracked = trackedOgmaState(cwd);
    if (tracked.length > 0) { refuseTrackedOgma('ingest', tracked, log); return 1; }
