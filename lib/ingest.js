...
const { verifyReceipt, makeGitReader } = require('./verify');
...
function cmdIngest(cwd, log = console.log) {
  try {
    ...
    if (head.error || head.status !== 0) { log('ogma ingest failed: not a git repository with commits.'); return 1; }
    ...
    fs.writeFileSync(path.join(ogham, 'manifest.json'), JSON.stringify(manifest, null, 2) + '\n');
