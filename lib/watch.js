  ...
  try {
    ...
    const changedFiles = (commit) => {
      if (!changedFilesAt.has(commit)) {
        const r = spawnSync('git', ['-C', cwd, 'diff', '--name-only', '--no-renames', commit, headCommit, '--'],
    ...
    const hunksFor = (commit, file) => {
      ...
      if (!hunksAt.has(key)) {
        const r = spawnSync('git', ['-C', cwd, 'diff', '-U0', '--no-renames', commit, headCommit, '--', file],
          { encoding: 'utf8', maxBuffer: 64 * 1024 * 1024 });
    ...
    // --- report --------------------------------------------------------------
    log(`Watched ${String(manifest.cutoff_commit).slice(0, 12)} → ${headCommit.slice(0, 12)}: ${checkedFacts} fact(s) checked.`);
