...
const { readJson, nowIso, ogmaWrite, gitHead, safeListFiles, MAX_REPORTED } = require('./util');
...
function parseHunkIntervals(diffText) {
  ...
  for (const line of String(diffText).split('\n')) {
    ...
    // delta is the line-count change this hunk makes. A hunk ABOVE a cited
    // range does not overlap it but still moves it by delta, which is why the
    // old side alone cannot answer "did this citation move".
    intervals.push({ start: Number(m[1]), count, delta: newCount - count });
  }
  return intervals;
}

// Does any hunk invalidate the widened receipt range [lo, hi]?
//
// TWO ways a citation stops describing the code it names, and both must be
// caught — under-invalidating certifies a stale receipt, which is the failure
// this whole mechanism exists to prevent:
//
//   1. OVERLAP. The hunk edits lines inside the range. Deletions and
//      modifications: plain interval overlap on the old side. Pure insertions
//      after old line a land between a and a+1, so they change the cited span
//      when a is inside [lo-1, hi] — conservative on the boundary.
//   2. SHIFT. The hunk sits entirely ABOVE the range and changes the line
//      count. It never overlaps, yet every cited line below it moves by
//      `delta`, so `file:44-46` now names different code. OGMA never
//      renumbers a receipt, so the only honest answer is to re-read the fact.
//      Missing this is how an insertion far above a citation left the gate
//      certifying PASS over a receipt pointing at unrelated lines.
//
// A hunk above with delta 0 (equal lines added and removed) leaves the
// numbering intact and does not invalidate: the cited lines are still the
// cited lines.
function hunksTouch(intervals, lo, hi) {
  for (const h of intervals) {
    ...
    const delta = h.delta === undefined ? 0 : h.delta;
    if (delta !== 0) {
      // Old-side end of this hunk; a pure insertion after `start` occupies no
      // old line, so it counts as ending at `start`.
      const oldEnd = count === 0 ? start : start + count - 1;
      if (oldEnd < lo) return true;
    }
  }
  return false;
}
...
// The currency checker — the ONE implementation of "is this fact's cited
// code untouched between its verified commit and HEAD". watch uses it to
// invalidate; ingest uses it to refuse advancing the cutoff over a fact
// whose citations moved (without this, graph -> ingest -> render -> gate
// after a commit certified PASS over a PRD contradicting HEAD, and watch —
// seeing cutoff == HEAD — was disarmed forever); the gate's freshness check
// uses it to re-prove the same thing at the certifying boundary.
// Returns staleReason(fact) -> string | null. null means provably current.
// Cannot-diff returns a reason, never null: silence is not freshness.
function makeCurrencyChecker(cwd, headCommit) {
  ...
  const changedFilesAt = new Map();   // commit -> Map(NFC name -> actual name) | { error }
  ...
  const changedFiles = (commit) => {
    if (!changedFilesAt.has(commit)) {
      ...
      if (r.error || r.status !== 0) {
        changedFilesAt.set(commit, { error: (r.stderr || 'git diff failed').trim() });
      } else {
        // Keyed by NFC per the normalization rule in docs/ogham-schema.md:
        // a macOS-written NFD name and an NFC-authored receipt must meet.
        // The ACTUAL diff name is kept as the value — the per-file diff
        // must hand git the repo's real bytes, not a normalized form.
        const byNfc = new Map();
        for (const p of r.stdout.split('\n').filter(Boolean)) byNfc.set(p.normalize('NFC'), p);
        changedFilesAt.set(commit, byNfc);
      }
    }
  ...
  return function staleReason(fact) {
    const commit = S.isCommitish(fact.verified_at_commit) ? fact.verified_at_commit : null;
    if (commit === null) return null;                    // not watched — the gate verifies these at HEAD
    if (S.sameCommit(commit, headCommit)) return null;   // verified at HEAD itself
    const changed = changedFiles(commit);
    if (changed.error) {
      return `verified_at_commit ${commit.slice(0, 12)} cannot be diffed against HEAD (${S.safe(changed.error, 120)})`;
    }
    for (const r of S.receiptsOf(fact)) {
      const actual = changed.get(String(r.file).normalize('NFC'));
      if (actual === undefined) continue;
      const hunks = hunksFor(commit, actual);
      const { lo, hi } = widenedRange(r);
      if (hunks.error) return `${r.file}: diff failed — currency cannot be proven`;
      if (hunksTouch(hunks, lo, hi)) return `${r.file}:${lo}-${hi} touched since ${commit.slice(0, 12)}`;
    }
    return null;
  };
}
...
function cmdWatch(cwd, log = console.log) {
  try {
    ...
    if (head.error) { log('ogma watch failed: not a git repository with commits.'); return 1; }
    ...
    // --- invalidate ----------------------------------------------------------
    // The diff machinery lives in makeCurrencyChecker (shared with ingest and
    // the gate — one implementation of the invalidation signal, never three).
    ...
    for (const { file, doc } of docs) {
      ...
      if (docChanged) {
        ogmaWrite(cwd, `ogham/facts/${file}`, JSON.stringify(doc, null, 2) + '\n');
        touchedDocs++;
      }
    }
    ...
    ogmaWrite(cwd, 'ogham/manifest.json', JSON.stringify(advanced, null, 2) + '\n');
...
module.exports = { cmdWatch, makeCurrencyChecker, parseHunkIntervals, hunksTouch, widenedRange };
